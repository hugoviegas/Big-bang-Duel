...
import type { AttackTimer, GameMode, Room as RoomType, RoomConfig } from '../../types';
...
const MODES = [
  { id: 'beginner' as GameMode, name: 'INICIANTE', color: 'border-green-600/40',  active: 'border-green-400 bg-green-900/30 text-green-300' },
  { id: 'normal'   as GameMode, name: 'NORMAL',    color: 'border-yellow-600/40', active: 'border-gold bg-gold/20 text-gold' },
  { id: 'advanced' as GameMode, name: 'AVANÇADO',  color: 'border-red-600/40',   active: 'border-red-400 bg-red-900/30 text-red-300' },
];
...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <button
      ...
      aria-label={label}
    >
      <span className={`absolute top-0.5 w-4 h-4 rounded-full shadow transition-all ${checked ? 'right-0.5 bg-black/80' : 'left-0.5 bg-sand/50'}`} />
    </button>
  );
}
...
export function OnlineLobby() {
  ...
  const { createRoom, joinRoom, getUserRooms, getPublicRooms } = useFirebaseRoom();
  ...
  const handleCreate = async () => {
    ...
    if (roomId) {
      useGameStore.getState().initializeGame(
        selectedMode, true, true, roomId, undefined, selectedChar,
  ...
  const handleJoin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (joinCode.length !== 6) { setError('Código deve ter 6 caracteres'); return; }
    const room = await joinRoom(joinCode.toUpperCase());
    if (room) {
      useGameStore.getState().initializeGame(
        room.mode, true, false, room.id, undefined, selectedChar,
        { attackTimer: room.config?.attackTimer ?? 10, bestOf3: room.config?.bestOf3 ?? false, isPublic: room.config?.isPublic ?? false }
      );
  ...
  const handleJoinPublic = async (room: RoomType) => {
    ...
    if (joined) {
      useGameStore.getState().initializeGame(
        room.mode, true, false, room.id, undefined, selectedChar,
        { attackTimer: room.config?.attackTimer ?? 10, bestOf3: room.config?.bestOf3 ?? false, isPublic: true }
      );
      navigate(`/game/${room.id}`);
    } else {
  ...
  const modeLabel = (m: GameMode) => ({ beginner: 'INICIANTE', normal: 'NORMAL', advanced: 'AVANÇADO' }[m]);
      ...
      <div className="relative z-10 w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-center gap-3">
              {CHARACTERS.map(char => (
                <button key={char.id} onClick={() => setSelectedChar(char.id)}
                  className={`flex flex-col items-center p-2 rounded-xl border-2 transition-all ${selectedChar === char.id ? 'border-gold bg-gold/15 scale-105' : 'border-transparent hover:border-sand/30'}`}
                >
          ...
          <div className="card-wood animate-fade-up animate-fade-up-delay-2 overflow-hidden">
            <button onClick={() => setShowAdvanced(v => !v)} className="w-full p-4 flex items-center justify-between">
              <h3 className="font-western text-[10px] text-sand/60 tracking-widest uppercase">3. Opções Avançadas</h3>
              <div className="flex items-center gap-1.5">
                {isPublic && (
                  <span className="text-[8px] font-stats uppercase px-1.5 py-0.5 rounded border border-sky-500/50 text-sky-400">Pública</span>
                )}
                {bestOf3 && (
                  <span className="text-[8px] font-stats uppercase px-1.5 py-0.5 rounded border border-purple-500/50 text-purple-400">MD3</span>
                )}
                {attackTimer !== 10 && (
                  <span className="text-[8px] font-stats uppercase px-1.5 py-0.5 rounded border border-orange-500/50 text-orange-400">{timerLabel(attackTimer)}</span>
                )}
                <svg className={`w-4 h-4 text-sand/50 transition-transform ${showAdvanced ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {showAdvanced && (
              <div className="px-4 pb-4 flex flex-col gap-5 border-t border-sand/10 pt-4">
                ...
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="font-western text-[11px] text-sand tracking-wide">Sala Pública</p>
                    <p className="font-stats text-[9px] text-sand/50 uppercase mt-0.5">Visível na lista de salas abertas</p>
                  </div>
                  <Toggle checked={isPublic} onChange={setIsPublic} label="Sala pública" />
                </div>

                {/* Best of 3 toggle */}
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="font-western text-[11px] text-sand tracking-wide">Melhor de 3</p>
                    <p className="font-stats text-[9px] text-sand/50 uppercase mt-0.5">Primeiro a vencer 2 rounds leva o duelo</p>
                  </div>
                  <Toggle checked={bestOf3} onChange={setBestOf3} label="Melhor de 3" />
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <p className="font-western text-[11px] text-sand tracking-wide">Tempo de Ataque</p>
                      <p className="font-stats text-[9px] text-sand/50 uppercase mt-0.5">Carta aleatória ao expirar o tempo</p>
                    </div>
                    <span className="font-western text-gold text-sm">{timerLabel(attackTimer)}</span>
                  </div>
                  <div className="flex gap-1.5">
                    {TIMER_OPTIONS.map(({ value, label }) => (
                      <button key={value} onClick={() => setAttackTimer(value)}
          ...
          <div className="card-wood p-6 animate-fade-up animate-fade-up-delay-3">
            {activeRooms.length > 0 ? (
              ...
              <button onClick={handleCreate} className="btn-western mb-4">CRIAR NOVINHA</button>
            )}
            ...
            <form onSubmit={handleJoin} className="flex flex-col gap-3">
              ...
              <button type="submit" className="btn-western btn-sky py-2 text-sm">ENTRAR</button>
            </form>
          </div>
        ...
        <div className="flex flex-col gap-4 animate-fade-up animate-fade-up-delay-3">
          ...
          <div className="flex rounded-xl overflow-hidden border border-sand/20">
            ...
            <button onClick={() => { setActiveTab('public'); loadAllRooms(); }}
            ...
            >
              ...
              {publicRooms.length > 0 && (
                <span className="text-[8px] bg-sky-500/30 border border-sky-500/50 text-sky-300 px-1.5 py-0.5 rounded-full font-stats">{publicRooms.length}</span>
              )}
            </button>
          </div>
            ...
            {loadingRooms ? (
                    ...
                    <div key={room.id} className="bg-black/40 border border-gold/20 rounded-xl p-3 flex justify-between items-center hover:border-gold/50 transition-all group">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[8px] px-1.5 py-0.5 rounded border font-stats uppercase ${room.status === 'waiting' ? 'border-yellow-500/50 text-yellow-500' : 'border-green-500/50 text-green-500'}`}>
                            {room.status === 'waiting' ? 'Aguardando' : 'Em Jogo'}
                          </span>
                        </div>
                        <span className="text-[10px] text-sand/50 font-stats uppercase">
                          {room.hostName} vs {room.guestName || '???'} • {modeLabel(room.mode)}
                        </span>
                        <div className="flex gap-1 flex-wrap">
                          {room.config?.isPublic && <span className="text-[7px] px-1 py-0.5 rounded border border-sky-500/40 text-sky-400 font-stats uppercase">Pública</span>}
                          {room.config?.bestOf3    && <span className="text-[7px] px-1 py-0.5 rounded border border-purple-500/40 text-purple-400 font-stats uppercase">MD3</span>}
                          {room.config?.attackTimer && <span className="text-[7px] px-1 py-0.5 rounded border border-orange-500/40 text-orange-400 font-stats uppercase">{room.config.attackTimer}s</span>}
                        </div>
                      </div>
              ...
              publicRooms.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-4 opacity-30">
                  ...
                  <span className="font-western text-xs text-center px-8">Nenhuma sala pública disponível no momento.</span>
                  <button onClick={loadAllRooms} className="font-stats text-[10px] text-gold/60 hover:text-gold uppercase tracking-widest mt-2">Atualizar</button>
                </div>
              ) : (
                <div className="space-y-3">
                  {publicRooms.map(room => (
                    <div key={room.id} className="bg-black/40 border border-sky-500/20 rounded-xl p-3 flex justify-between items-center hover:border-sky-500/50 transition-all group">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          <span className="font-western text-sky-300 text-base tracking-widest">{room.id}</span>
                          <span className="text-[8px] px-1.5 py-0.5 rounded border border-yellow-500/50 text-yellow-500 font-stats uppercase">Aguardando</span>
                        </div>
                        <span className="text-[10px] text-sand/50 font-stats uppercase">
                      ...
                      <button onClick={() => handleJoinPublic(room)}
          ...
          <button onClick={loadAllRooms} className="text-center text-sand/30 font-stats text-[9px] uppercase tracking-widest hover:text-sand/60 transition-colors">
            ↻ Atualizar listas
          </button>
        </div>

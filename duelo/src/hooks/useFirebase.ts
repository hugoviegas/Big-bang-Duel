...
export function useFirebaseRoom() {
  ...
  return { createRoom, joinRoom, currentRoomId };
}
...
export function useMatchSync(roomId: string | null) {
  ...
  useEffect(() => {
    ...
    onDisconnect(presenceRef).set('finished'); // Simple disconnect handling
    ...
    const unsubscribe = onValue(roomRef, (snapshot) => {
      ...
      // Update GameStore from Firebase if needed
    ...
    return () => {
      unsubscribe();
      off(roomRef);
      // Cancel disconnect if naturally unmounting? For now let's keep it simple.
    };
  }, [roomId, user]);
}

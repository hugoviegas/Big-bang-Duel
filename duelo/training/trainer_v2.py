#!/usr/bin/env python3
"""
Big Bang Duel — Trainer V2: Pattern-Aware Q-Learning
=====================================================
Melhorias em relação à v1 (trainer_ultra.py):

1. ESTADO ESTENDIDO — inclui a carta anterior do oponente (prev_opp_card):
   Estado: {my_life}_{my_ammo}_{opp_life}_{last_opp}_{prev_opp}_{my_dodge}_{my_double}
   Permite que o bot detecte padrões de 2 turnos do jogador
   (ex: o jogador fez reload→reload? Bot aprende a atirar)

2. FILTRO DE DODGE — dodge_streak integrado na tabela de ações pré-computada
   Corrige bug onde o bot podia esquivar 4+ vezes consecutivas

3. PENALIDADE DE REPETIÇÃO — o bot perde recompensa se repetir a mesma
   carta 3 vezes seguidas, forçando variação nas jogadas

4. INJEÇÃO DE OPONENTE ALEATÓRIO — 25% dos episódios, um lado joga
   aleatoriamente, garantindo robustez contra qualquer estratégia

5. DIFICULDADES MELHORADAS:
   - hard  (temp=0.2): muito decisivo, lê padrões
   - medium(temp=2.5): adapta-se mas erra às vezes
   - easy: 55% pior jogada + 45% aleatório (erra de propósito, mas ainda
     reage a padrões do jogador de forma atrasada/errada)

Estado key format: "{my_life}_{my_ammo}_{opp_life}_{last_opp_card}_{prev_opp_card}_{my_dodge_streak}_{my_double_shots_left}"
Exemplo: "3_2_2_shot_reload_0_3"
"""
...
# ─── Penalty for 3-in-a-row same card ─────────────────────────────────────
REP_PENALTY = -1.5
...
class PatternAwareTrainer:
    """
    Q-Learning com estado estendido por histório de 2 cartas do oponente.
    Treinamento por self-play com injeção ocasional de oponente aleatório.
    """
    ...
    def __init__(
        ...
        self.random_opp_ratio = random_opp_ratio  # fração de episódios com um lado aleatório
    ...
    def _build_mode_spec(self, mode: str) -> ModeSpec:
        ...
        # Estado estendido: adicionamos prev_opp (segunda dimensão last_count)
        state_count = (
            max_life
            * (MAX_AMMO + 1)
            * max_life
            * last_count   # last_opp
            * last_count   # prev_opp  ← NOVO
            * (MAX_DODGE_STREAK + 1)
    ...
    def _build_precomputed_actions(self):
        """Pré-computa cols disponíveis para (ammo, double_left, dodge_streak)."""
    ...
    def _state_index(
        ...
        my_ammo: int,
        opp_life: int,
        last_opp: int,   # código da última carta do oponente
        prev_opp: int,   # código da penúltima carta do oponente (NOVO)
        my_dodge: int,
        my_double: int,
    ) -> int:
        lc = self.spec.last_count
        ml = self.spec.max_life
        idx = my_life - 1
        idx = idx * (MAX_AMMO + 1) + my_ammo
        idx = idx * ml          + (opp_life - 1)
        idx = idx * lc          + last_opp
        idx = idx * lc          + prev_opp    # NOVO
    ...
    def _resolve_turn(
        ...
        """Returns (p_life_lost, o_life_lost, p_ammo_change, o_ammo_change)."""
    ...
    def _reward(
        ...
        o_life_lost: int,
        winner: int | None,
        perspective: int,     # 1=player, -1=opponent
        ...
        r = opp_lost * 5.0 - my_lost * 5.0 - 0.3 + rep_penalty
    ...
    def _initial_state(self) -> dict:
        ...
        return {
            ...
            'last_p': 0,  'last_o': 0,  # códigos da última carta
            'prev_p': 0,  'prev_o': 0,  # códigos da penúltima carta (NOVO)
            'last_p_act': -1, 'last_o_act': -1,  # action ids para anti-rep
    ...
    def _random_start_state(self) -> dict:
        ...
        s.update({
            ...
            'last_p': self.rng.randrange(lc),
            'last_o': self.rng.randrange(lc),
            'prev_p': self.rng.randrange(lc),
            'prev_o': self.rng.randrange(lc),
    ...
    def _play_episode(self, epsilon: float, alpha: float) -> EpisodeResult:
        ...
        # Decide se um lado vai jogar aleatoriamente neste episódio
        ep_rand = self.rng.random()
        rand_p = ep_rand < self.random_opp_ratio / 2
        rand_o = self.random_opp_ratio / 2 <= ep_rand < self.random_opp_ratio
        ...
        while True:
            ...
            p_sidx = self._state_index(
                ...
                s['last_o'], s['prev_o'], s['p_dodge'], s['p_double']
            )
            o_sidx = self._state_index(
                s['o_life'], s['o_ammo'], s['p_life'],
                s['last_p'], s['prev_p'], s['o_dodge'], s['o_double']
            )
            ...
            # ── Penalidade de repetição (3x mesma carta consecutiva) ──────
            ...
            r_p = self._reward(p_ll, o_ll, winner, perspective=1,  rep_penalty=p_rep)
            r_o = self._reward(p_ll, o_ll, winner, perspective=-1, rep_penalty=o_rep)

            # ── Atualizar cartas anteriores ───────────────────────────────
            s['prev_p'] = s['last_p']
            s['prev_o'] = s['last_o']
    ...
    def train(self):
        ...
        if self.verbose:
            print(f"\n{'=' * 70}")
            print(f"PATTERN-AWARE TRAINER V2 | mode={self.mode} | episodes={self.episodes:,}")
...
def _train_worker_v2(payload: dict) -> dict:
    trainer = PatternAwareTrainer(
        mode=payload['mode'],
...
def train_parallel_v2(
    ...
    with get_context('spawn').Pool(processes=workers) as pool:
        results = pool.map(_train_worker_v2, payloads)

    merged = PatternAwareTrainer(
        mode=mode, episodes=episodes,
        alpha=alpha, gamma=gamma, epsilon=epsilon,
        random_opp_ratio=random_opp_ratio, seed=seed,
    )
...
def generate_strategies_v2(trainer: PatternAwareTrainer) -> dict:
    """
    Gera estratégias hard/medium/easy a partir das Q-tables treinadas.

    Dificuldades:
      hard   — temperatura 0.2: muito decisivo, lê padrões com precisão
      medium — temperatura 2.5: parcialmente adaptativo, erra às vezes
      easy   — 55% pior jogada + 45% aleatório: erra de propósito
                mas ainda reage fracamente a padrões
    """
            ...
            for opp_life in range(1, spec.max_life + 1):
                for my_dodge in range(MAX_DODGE_STREAK + 1):
                    for my_double in range(MAX_DOUBLE_SHOT_USES + 1):
                        ...
                        for last_name in spec.last_card_names:
                            last_code = spec.last_name_to_code[last_name]

                            for prev_name in spec.last_card_names:  # NOVO loop
                                prev_code = spec.last_name_to_code[prev_name]
                                ...
                                key = (
                                    f"{my_life}_{my_ammo}_{opp_life}"
                                    f"_{last_name}_{prev_name}"
                                    f"_{my_dodge}_{my_double}"
                                )
...
def validate_v2(
    ...
    """
    Valida o nível 'hard' jogando contra um oponente aleatório.
    Retorna taxa de vitória do bot, empates e turnos médios.
    """
    from game_engine import CARDS_BY_MODE, LIFE_BY_MODE, MAX_AMMO, MAX_TURNS
    ...
    for _ in range(num_games):
        ...
        last_o = 'none'; prev_o = 'none'
        turn = 1

        while True:
            # Bot (jogador) — usa estratégia hard
            key = f"{p_life}_{p_ammo}_{o_life}_{last_o}_{prev_o}_{p_dodge}_{p_double}"
            ...
            # Resolver
            from game_engine import resolve_cards
...
def write_output_v2(
    ...
    payload = {
        'meta': {
            ...
            'version': '3.0-pattern-aware',
            'state_format': '{my_life}_{my_ammo}_{opp_life}_{last_opp_card}_{prev_opp_card}_{my_dodge_streak}_{my_double_shots_left}',
            'engine': 'numpy-indexed-qtable-v2-pattern-aware',
            'features': [
                'extended_state_prev_opp_card',
...
def main():
    parser = argparse.ArgumentParser(
        description='Big Bang Duel — Pattern-Aware Q-Learning Trainer V2',
    ...
    parser.add_argument('--output',           type=str,   default='output_v2')
    ...
    parser.add_argument('--random-opp-ratio', type=float, default=0.25,
                        help='Fração de episódios com um lado aleatório (anti-overfitting)')
    args = parser.parse_args()

    max_workers = max(1, cpu_count() - 1)
    workers = min(max(1, args.workers), max_workers)

    trainer = train_parallel_v2(
        mode=args.mode,
    ...
    print('\nGerando estratégias...')
    strategies = generate_strategies_v2(trainer)

    if args.skip_validation:
        validation = {'skipped': True}
    else:
        print(f'\nValidando ({args.validation_games:,} jogos)...')
        validation = validate_v2(strategies, args.mode, num_games=args.validation_games)
    ...
    write_output_v2(trainer, strategies, validation, out)
